  ...
  <AdminLayout>
    <div class="max-w-7xl mx-auto py-10 px-4">
      <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold text-brown-800">Gestão de Serviços</h1>
        <Link :href="route('services.create')" class="bg-brown-600 text-white px-4 py-2 rounded hover:bg-brown-700">+ Serviço</Link>
      </div>
        ...
        <div v-for="service in services.data" :key="service.uuid" class="bg-white rounded shadow overflow-hidden">
          ...
          <div class="p-4 space-y-2">
            ...
            <div class="flex gap-2 mt-2">
              <Link :href="route('services.edit', service.uuid)" class="px-3 py-1 text-sm bg-yellow-500 text-white rounded hover:bg-yellow-600">Editar</Link>
              <Link as="button" method="delete" :href="route('services.destroy', service.uuid)" class="px-3 py-1 text-sm bg-red-600 text-white rounded hover:bg-red-700" preserve-scroll>Excluir</Link>
            </div>
          </div>
        </div>
